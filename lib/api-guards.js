...
// Set generously on purpose. A script hammering these endpoints makes hundreds
// of requests, so anything in this range stops it dead. A real buyer, meanwhile,
// can easily rack up attempts through no fault of their own: picking slots
// someone else just took, fumbling the upload, retrying after a bad connection.
// Every one of those counts. Turning away a buyer holding a card costs far more
// than letting an attacker have a few extra requests before the wall.
export const LIMITS = {
  reserve: { max: 15, windowMinutes: 10 },
  checkout: { max: 20, windowMinutes: 10 }
};
...
// Proves that whoever is asking to check out is the same person who created
// the reservation, without needing accounts, sessions or a database column.
//
// /api/reserve hands this token back to the buyer's browser, and /api/checkout
// will not touch a purchase without it. It is derived from the purchase id and
// a server-side secret, so it can be recomputed on demand but not guessed.
// Without it, purchase ids run 1, 2, 3 and anyone can walk them.
export async function purchaseToken(env, purchaseId) {
  const secret = env.RATE_LIMIT_SALT || 'sat-space-unsalted';
  const key = await crypto.subtle.importKey(
    'raw', new TextEncoder().encode(secret), { name: 'HMAC', hash: 'SHA-256' }, false, ['sign']
  );
  const sig = await crypto.subtle.sign('HMAC', key, new TextEncoder().encode('purchase:' + purchaseId));
